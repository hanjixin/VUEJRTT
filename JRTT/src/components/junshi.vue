...
<script>
  ...
  export  default {
    ...
    methods:{
      getList:function () {
        this.$http.get('list/?tag=news_military&ac=wap&count=20&format=json_raw&as=A175B96D83E399F&cp=59D3137919DFEE1&min_behot_time='+(new Date().getTime()+'').slice(0,9)).then(res=>{
          console.log(res)
          if(res.data.data){
