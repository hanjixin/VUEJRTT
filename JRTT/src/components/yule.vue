...
<script>
  ...
  export  default {
    ...
    methods:{
      getList:function () {
        this.$http.get('list/?tag=news_entertainment&ac=wap&count=20&format=json_raw&as=A175390D03E3947&cp=59D373997417FE1&min_behot_time='+(new Date().getTime()+'').slice(0,9)).then(res=>{
          console.log(res)
          if(res.data.data){
