...
<script>
  ...
  export  default {
    ...
    methods:{
      getList:function () {
        this.$http.get('list/?tag=news_society&ac=wap&count=20&format=json_raw&as=A1D5F9ADD3037E1&cp=59D35367FED16E1&min_behot_time='+(new Date().getTime()+'').slice(0,9)).then(res=>{
          console.log(res)
          if(res.data.data){
