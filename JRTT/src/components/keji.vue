...
<script>
  ...
  export  default {
    ...
    methods:{
      getList:function () {
        this.$http.get('list/?tag=news_tech&ac=wap&count=20&format=json_raw&as=A1E5D9FDC343B7A&cp=59D3F3ABC7BA7E1&min_behot_time='+(new Date().getTime()+'').slice(0,9)).then(res=>{
          console.log(res)
          if(res.data.data){
