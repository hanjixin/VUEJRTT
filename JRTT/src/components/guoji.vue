...
<script>
  ...
  export  default {
    ...
    methods:{
      getList:function () {
        this.$http.get('list/?tag=news_world&ac=wap&count=20&format=json_raw&as=A125D92D8323DDF&cp=59D3538D9DBFBE1&min_behot_time='+(new Date().getTime()+'').slice(0,9)).then(res=>{
          console.log(res)
          if(res.data.data){
